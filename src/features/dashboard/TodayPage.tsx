...
export default function TodayPage() {
  ...
  if (habitsLoading || loading) {
    return <div className="pt-16 text-center text-sm text-zinc-400">Cargando tu día…</div>
  }
  ...
  return (
    <div className="space-y-5">
      <header>
        ...
        <h1 className="text-2xl font-bold text-zinc-900">Tu día</h1>
      </header>
      ...
      <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
        <div className="mb-2 flex items-end justify-between">
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-zinc-400">Progreso de hoy</p>
        ...
        {dayPct === 100 && totalExpected > 0 && (
          <p className="mt-2 text-sm font-medium text-emerald-600">🎉 ¡Día completado!</p>
        )}
      </section>
      ...
      <section className="space-y-3">
        {grouped.length === 0 && (
          <div className="rounded-2xl border border-dashed border-zinc-300 p-8 text-center">
            <p className="text-sm text-zinc-500">Aún no tienes hábitos.</p>
            <Link
              to="/habits"
        ...
        {grouped.map(({ habit, occs, done, total, pct }) => (
          <article key={habit.id} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
            <div className="mb-2 flex items-center justify-between">
              ...
              {total > 0 && pct === 100 ? (
                <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">Listo ✓</span>
              ) : (
            ...
            {occs.length > 0 && (
              <ul className="mt-1 space-y-1.5">
                {occs.map((o) => {
                  ...
                  return (
                    <li key={o.id} className="flex items-center justify-between rounded-xl bg-zinc-50 px-3 py-2">
                      <div className="flex items-center gap-2.5">
                        <span
                          className={cn(
                        ...
                        <span className={cn('text-sm', o.status === 'completed' ? 'text-zinc-400 line-through' : 'text-zinc-700')}>
                      ...
                      <div className="flex gap-1.5">
                        ...
                        {o.status === 'pending' && (
                          <button
                            ...
                            className="rounded-lg bg-zinc-200 px-2.5 py-1.5 text-xs font-medium text-zinc-600 transition active:scale-95"
                          >
                            Omitir
                          </button>
                        )}
                        {o.status !== 'pending' && (
                          <button
                            ...
                            className="rounded-lg bg-zinc-100 px-2.5 py-1.5 text-xs font-medium text-zinc-500 transition active:scale-95"
                            title="Deshacer"
                          >
