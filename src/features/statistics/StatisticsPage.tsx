...
export default function StatisticsPage() {
  ...
  if (habitsLoading || loading) return <div className="pt-16 text-center text-sm text-zinc-400">Calculando…</div>

  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-2xl font-bold text-zinc-900">Estadísticas</h1>
        <p className="text-sm text-zinc-500">Tu constancia en números</p>
      </header>
      ...
      <div className="grid grid-cols-2 rounded-xl bg-zinc-200/70 p-1 text-sm font-medium">
        {(['week', 'month'] as const).map((r) => (
          <button
            key={r}
            onClick={() => setRange(r)}
            className={cn('rounded-lg py-2 capitalize transition', range === r ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-500')}
          >
            {r === 'week' ? 'Semana' : 'Mes'}
          </button>
        ))}
        ...
        <>
          ...
          <section className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
              <p className="text-xs font-medium uppercase tracking-wide text-zinc-400">Cumplimiento</p>
              <p className="mt-1 text-3xl font-bold text-zinc-900">{data.pct ?? 0}%</p>
            </div>
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
              <p className="text-xs font-medium uppercase tracking-wide text-zinc-400">Racha actual</p>
              <p className="mt-1 text-3xl font-bold text-violet-700">🔥 {data.streak} días</p>
            </div>
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
              <p className="text-xs font-medium uppercase tracking-wide text-zinc-400">Mejor racha</p>
              <p className="mt-1 text-3xl font-bold text-zinc-900">{data.best} días</p>
            </div>
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
              <p className="text-xs font-medium uppercase tracking-wide text-zinc-400">Hábitos activos</p>
              <p className="mt-1 text-3xl font-bold text-zinc-900">{habits.filter((h) => h.is_active).length}</p>
            </div>
          </section>
              ...
              {dayLabels.map(([d, { expected, done }]) => {
                ...
                return (
                  <div key={d} className="flex flex-1 flex-col items-center gap-1">
                    <div className="flex h-24 w-full items-end overflow-hidden rounded-md bg-zinc-100">
                      <div
                        className={cn('w-full rounded-md transition-all duration-500', pct === 100 ? 'bg-emerald-500' : pct > 0 ? 'bg-violet-500' : 'bg-zinc-200')}
                        style={{ height: `${Math.max(pct, 3)}%` }}
                      />
                    </div>
                    <span className="text-[9px] font-medium text-zinc-400">{d.slice(8, 10)}</span>
                  </div>
                )
              })}
            </div>
          ...
          <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-100">
            ...
            {data.perHabit.length === 0 ? (
              <p className="text-sm text-zinc-400">Sin hábitos activos en este período.</p>
            ) : (
